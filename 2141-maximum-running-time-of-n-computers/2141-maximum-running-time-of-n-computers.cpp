class Solution {
public:
    long long int maxRunTime(int n, std::vector<int>& batteries) {
        ...
        std::vector<int> live(batteries.begin() + batteries.size() - n, batteries.end());
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            if (extra < static_cast<long long int>(i + 1) * (live[i + 1] - live[i])) {
                return live[i] + extra / static_cast<long long int>(i + 1);
            }

            // Reduce 'extra' by the total power used.
            extra -= static_cast<long long int>(i + 1) * (live[i + 1] - live[i]);
        }

        // If there is power left, we can increase the running time
        // of all computers.
        return live[n - 1] + extra / static_cast<long long int>(n);
    }
};
